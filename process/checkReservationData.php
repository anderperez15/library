...
$dataL=mysqli_fetch_array($selectDataL);
if(mysqli_num_rows($selectDataL)>=1){
    echo '<div class="alert alert-info text-center" role="alert"><strong><i class="zmdi zmdi-alert-triangle"></i> &nbsp; ¡Importante!:</strong> Para aprobar el préstamo selecciona el código correlativo de la carpeta a prestar y haz click el botón "Aprobar reservación"</div>';
    $selectDataB=ejecutarSQL::consultar("SELECT * FROM libro WHERE CodigoLibro='".$dataL['CodigoLibro']."'");
    $fila=mysqli_fetch_array($selectDataB);
    echo '<div class="group-material"><span>Código correlativo</span><select class="tooltips-general material-control" name="bookCorrelative" data-toggle="tooltip" data-placement="top" title="Elige el código correlativo de la carpeta">';
