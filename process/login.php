...
if($UserType=="Student"){
    ...
    $fila=mysqli_fetch_array($checkStudent);
...
if($UserType=="Teacher"){
    ...
    mysqli_free_result($checkTeacher);
}
if($UserType=="Admin"){
	...
	$fila=mysqli_fetch_array($checkAdmin);
    if(mysqli_num_rows($checkAdmin)>0){
        ...
        echo '<script type="text/javascript">
            swal({ 
                ...
                text:"Verifique sus datos y su tipo de usuario e intente nuevamente, o póngase en contacto con el administrador del archivo", 
                type: "error", 
                confirmButtonText: "Aceptar" 
            });
        </script>';
...
if($UserType=="Personal"){
    ...
    $fila=mysqli_fetch_array($checkPersonal);
    if(mysqli_num_rows($checkPersonal)>0){
        $selectBit=ejecutarSQL::consultar("SELECT * FROM bitacora");
        $total=mysqli_num_rows($selectBit)+1;
        $longitud=4; 
        for ($i=1; $i<=$longitud; $i++){ 
    ...
    mysqli_free_result($checkPersonal);
}
if($UserType==""){
    echo '<script type="text/javascript">
        swal({ 
